   ...
   where TDocument : IDocument
{
    ...
    public async Task<IList<TDocument>> FilterByAsync(
        Expression<Func<TDocument, bool>> filterExpression)
    {
        var response = await _collection.FindAsync(filterExpression);
    ...
    {
        var document = await _collection.Find(filterExpression).ToListAsync();
    ...
    public virtual async Task<TDocument> FindOneByFilterAsync(FilterDefinition<TDocument> filterExpression)
    {
        var document = await _collection.Find(filterExpression).ToListAsync();
        return document.OrderByDescending(x => x.Id).FirstOrDefault();
        //return Task.Run(() => 
